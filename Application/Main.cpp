...
{
	// create engine
	std::unique_ptr<gme::Engine> engine = std::make_unique<gme::Engine>();
	engine->Startup();
	engine->Get<gme::Renderer>()->Create("OpenGL", 800, 600);

	// create scene
	std::unique_ptr<gme::Scene> scene = std::make_unique<gme::Scene>();
	...
	// create model
	{
		...
		component->model = engine->Get<gme::ResourceSystem>()->Get<gme::Model>("models/cube.obj");
		component->material = engine->Get<gme::ResourceSystem>()->Get<gme::Material>("materials/wood.mtl", engine.get());
